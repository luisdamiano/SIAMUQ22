...
\title{\textbf{Automatic Dynamic Relevance Determination} \\
  ...
  for high-dimensional functional input model emulation}
...
\date[January 31st, 2022]{\tiny{NASA JPL MLS UQ tagup}\\ January 31st, 2022}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Can we connect the functional input structure to a physical
    mechanism?
  \item Can we integrate the functional input structure into the GP?
  \item Can we circumvent input dimension reduction?
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{tikzpicture}
    ...
    {
      Permutation\\Feature\\\textit{Dynamic Importance}
    };
...
\begin{frame}
  ...
  \begin{tikzpicture}
    ...
    \node (aspect2)    [below=aspect1b]                 [text width=15ex] {
      Distance\\{\footnotesize \itshape (dissimilarity)}
    };
    \node (aspect3)    [below=aspect2]                  [text width=15ex] {
...
\begin{frame}
  ...
  \begin{columns}[t]
	...
	are similar everywhere \emph{but especially} near $\tau$
        \\ \uwave{circumvent input dimension reduction}
      \item<6-> Priors go here
      \end{itemize}
    \end{column}
  \end{columns}
...
\myHearts{NASA's Microwave Limb Sounder\\\footnotesize\emph{a case study}}
...
%   \item Nine plausible model configurations
...
%   \item 1,500 posterior samples total
%   \item No large scale approximation
...
\begin{frame}
  ...
  \begin{itemize}
  \item[+]<1-> High dimensional inputs input with no dimension reduction
    \begin{itemize}
    \item Reduce $340 \to 45$ unknowns compared to a full GP
    \item Scales up for applications with higher input resolution
    \end{itemize}
  \item[+]<2-> Explicit link output correlation and
    input functional structure
    \begin{itemize}
    \item<2-> Can incorporate domain-specific knowledge, if any
    \item<2-> Interpretation $\to$ insight?
    \item<2-> Smooths out erratic relevance patterns
    \end{itemize}
  \item[++]<3-> Extensible to \alert{\textbf{complex
